...
/* PSO Encryption Library (BlueBurst Encryption)
 * 
 ...
 *   library; lobbylurk may in future support BB but it's unlikely
 */

unsigned long bbtable[] = {
...
void CRYPT_BB_Decrypt(CRYPT_SETUP* cs,void* vdata,unsigned long length)
{
    ...
    unsigned long eax,ecx,edx,ebx,ebp,esi,edi;
...
void CRYPT_BB_CreateKeys(CRYPT_SETUP* cs,void* seed)
{
    ...
    memcpy(cs->keys,bbtable,sizeof(bbtable));
    ...
    while (ebx < 0x12)
    {
        ...
        eax = ecx + 2;
        edx = eax - ((eax / 48) * 48);
        eax = (((unsigned long)s[edx]) << 8) & 0xFF00;
